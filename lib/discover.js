...
var reservedEvents = ['promotion', 'demotion', 'added', 'removed', 'master', 'masterDemoted', 'hello'];
...
function Discover (options, callback) {
	...
	self.evaluateHello = function (data, obj, rinfo) {
		...
		if (node.isMaster) {
			...
			self.emit("masterDemoted", node, obj, rinfo);
		}

		self.emit("helloReceived", node, obj, rinfo, isNew, wasMaster);
	};

	self.broadcast.on("hello", self.evaluateHello.bind(self));
